...
public class BallMoveThread extends Thread {
    ...
    public void run() {
        while (flag) {
            ...
            //确定位置和旋转角
            ball.ballX = ball.ballX + ball.ballVX * t + ballGX * t * t / 2;//VT+1/2A*T*T
    ...
    public Boolean knockWall(float ballX, float ballZ, double xForward, double zForward) {
        ...
        //如果向X轴正方向走
        if (xForward > 0) {
            for (int i = (int) ((ballX + ballR) / UNIT_SIZE); i <= (int) ((ballX + ballR + xForward) / UNIT_SIZE); i++) {//循环，假如它一下穿过几个格子，那么从第一个格子开始判断
